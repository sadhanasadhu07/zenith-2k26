...
export default function CommitteeSection() {
    ...
    return (
        <section id="committee" className="py-24 bg-cyan-950 relative overflow-hidden">
            {/* Ambient glows */}
            <div className="absolute top-0 right-0 w-[600px] h-[500px] bg-amber-500/5 rounded-full blur-[130px] pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-[600px] h-[400px] bg-cyan-500/5 rounded-full blur-[130px] pointer-events-none" />
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                >
                    ...
                    <div className="flex items-center justify-center gap-3 mb-4">
                        <div className="w-8 h-[2px] bg-amber-500/40 rounded-full" />
                ...
                {grouped.map((group, gIdx) => (
                    <div key={group.label} className="mb-14 last:mb-0">
                        ...
                        <motion.p
                            ...
                            className={`text-center font-mono text-xs tracking-widest uppercase mb-6 ${groupAccent[group.label] ?? "text-slate-400"}`}
                        >
                            {group.label}
                        </motion.p>
                        ...
                        >
                            {group.members.map((member) => (
                                <motion.div
                                    ...
                                    className="flex flex-col items-center text-center w-44 cursor-default"
                                >
                                    {/* Avatar */}
                                    <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-white/10 hover:border-amber-500/50 transition-all duration-300 mb-4 hover:shadow-[0_0_18px_rgba(245,158,11,0.2)]">
                                        <img
                                            src={member.image}
                                            alt={member.name}
                                    ...
                                    {/* Role */}
                                    <p className={`text-xs font-medium uppercase tracking-wider ${groupAccent[group.label] ?? "text-slate-400"}`}>
                                        {member.role}
                                    </p>
                                </motion.div>
                            ))}
                        ...
                        {gIdx < grouped.length - 1 && (
                            <motion.div
                                ...
                                className="mt-12 h-px w-64 mx-auto bg-gradient-to-r from-transparent via-white/10 to-transparent"
                            />
                        )}
                    </div>
                ))}
